  ...
  def summingPairsHelper(xs: Vector[Int],
    ...
    xs match {
      case fst +: rest =>
        ...
        val pairs_here: Future[Vector[Tuple2[Int,Int]]] = Future{rest.collect({case snd if fst + snd <= sum => (fst,snd)})}
        // Make the recursive call, adding in the pairs we just found.
        summingPairsHelper(rest, pairs_here.flatMap(the_pairs_here => the_pairs.flatMap(the_future_pairs => Future {the_future_pairs ++ the_pairs_here})))
