...
val variables = List("a","b","c","d","e","f","g","h","i","j")

def chooseVar(value: Int) : String = value < 10 match {
  case true => variables(value)
  case _ => chooseVar((value - 10)/ 10) + chooseVar(value  % 10)
}



def walk(currentBinders: Int, t: ULTerm): Int = t match {
  case ULVar(x) => currentBinders

  case ULAbs(t) =>
    walk(currentBinders+1, t)

  case ULApp(t1,t2) => currentBinders
}

def prettify(x: ULTerm) : String = x match {
  case ULVar(index) => chooseVar(index)
  case ULApp(t1, t2) => "(" + prettify(t1) + ") (" + prettify(t2) + ")"
  case ULAbs(t) => "lambda " + chooseVar(walk(0,t)) + " . " + prettify(t)
}
